...
def writer():
    ...
    if st.button("点击开始生成"):
        ...
        for i in range(num):
            ...
            st.text(content[i])
            # 创建毛笔字图片
            calligraphy_image = draw_calligraphy(content[i])
            # 显示图片
            st.image(calligraphy_image, caption="生成图片", use_column_width=True)
            # st.text(content[i])
    else:
        st.stop()

def draw_calligraphy(text):
    # 设置图片尺寸和背景颜色
    ...
    line_height = font.getsize(lines[0])[1]  # 假设所有行高相同
