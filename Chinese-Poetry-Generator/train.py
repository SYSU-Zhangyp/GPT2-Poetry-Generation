...
def build_files(data_path: str, tokenized_data_path: str, num_pieces: int,
    ...
    # 读取训练原始语料中的诗歌格式和诗句内容
    form, title, dynamic, author, content= np.loadtxt(data_path, delimiter=',', 
        skiprows=1, unpack=True, usecols=(0, 1, 2, 3, 4), encoding='utf-8', dtype=str)
    # 把每行数据统一格式：格式[SEP]标题[SEP]朝代[SEP]作者[SEP]诗歌内容
    lines = [ ]
    for i in range(len(form)):
        line = form[i] + '[SEP]' + title[i] + '[SEP]' +  dynamic[i] + '[SEP]'+ author[i] + '[SEP]'+ content[i].replace('\n', '[SEP]')
        lines.append(line)
